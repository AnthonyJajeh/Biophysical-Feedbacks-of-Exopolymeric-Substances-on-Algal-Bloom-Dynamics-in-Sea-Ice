...
gamma = .01; 
nu = .2;
eta_vec = linspace(.03*.5,.03*1.5,50); 
xi = .2;
rho = .75; 
delta = .007;
...
A_max = zeros(1, length(eta_vec));
N_max = zeros(1,length(eta_vec));
E_max = zeros(1,length(eta_vec));
...
% Plot nutrients on the left y-axis
yyaxis left;
plot(eta_vec, N_max, 'color', nutrientcolordet, 'linewidth', 3);
plot(eta_vec, A_max, 'color', algaecolordet, 'linewidth', 3,'LineStyle','-');
...
yyaxis right;
hold on;
plot(eta_vec, E_max, 'color', EPScolordet, 'linewidth', 3);
